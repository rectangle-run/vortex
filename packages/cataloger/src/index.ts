...
const confirmation = await confirm({
	message: "Are you sure you want to continue? Cataloger is in beta, so you should make sure you have backups of your data.",
});
...
for (const name in collectedVersions) {
	const uniqueVersions = new Set(unwrap(collectedVersions[name]).filter(x => !isCatalog(x)));
...
await Bun.write(join(root, "package.json"), JSON.stringify(rootPkg, null, "\t"));
...
console.log("All package.json files have been updated with the resolved versions.");
