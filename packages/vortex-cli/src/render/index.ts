...
export class Canvas {
	...
	put(x: number, y: number, cell: Cell) {
		...
		if (rx < this.clipStartX || rx >= this.clipEndX || ry <
			this.clipStartY || ry >= this.clipEndY) {
			return;
		}

		const index = ry * this.width + rx;
	...
	clip(x1: number, y1: number, x2: number, y2: number) {
		...
		this.clipStartX = Math.max(0, x1 + this.offsetX, this.clipStartX);
		this.clipStartY = Math.max(0, y1 + this.offsetY, this.clipStartY);
		this.clipEndX = Math.min(this.width, x2 + this.offsetX, this.clipEndX);
		this.clipEndY = Math.min(this.height, y2 + this.offsetY, this.clipEndY);
...
export class Renderer {
	...
	render(canvas: Canvas) {
		...
		this.ansi.setCursorVisible(false);
	}
}
