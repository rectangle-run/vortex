...
import type { ModuleDeclaration, Node, Span } from "oxc-parser";
...
export class Transformer {
	...
	getExportId(span: Span) {
		return `$d_${span.start}_${span.end}`;
	}
	...
	transform(node: Node) {
		...
		for (const key in node) {
			...
			if (Array.isArray(data)) {
				const newArray = this.transformArray(data as any);

				(node as any)[key] = newArray;
			} else if (isNode(data)) {
				const next = this.transformArray([data]);

				(node as any)[key] = next[0];
			}
		}
