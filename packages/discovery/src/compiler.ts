...
type OutputData = Omit<Awaited<ReturnType<typeof discoveryCompile>>, `${"client" | "server"}Eligible`>
...
export async function discoveryCompile(props: DiscoveryProps): Promise<{
    ...
    const inputHash = nonConstantFastHash(JSON.stringify({
        jsx: props.jsx,
        ts: props.typescript,
        code: props.source
    })).toString(36);

    const output = props.cache?.query?.(discoveryTransformSpec, {
        inputHash,
