...
export namespace SKL {
	...
	export function* lex(str: string): Generator<Token, any, any> {
		...
		while (i < str.length) {
			...
			throw new Error(
				`Unknown character: '${indicator}' at position ${i}`,
			);
		}
	}
		...
		if (indicator[0] === "symbol" && indicator[1] === "LeftParenthesis") {
			...
			while (true) {
				...
				if (key[0] !== "identifier" && key[0] !== "string") {
					throw new Error(
						`Expected identifier or string, got ${key[0]}`,
					);
				}
			...
			if (clazz !== null) {
				...
				if (clazz === "map") {
					...
					for (const [key, value] of kv.entries as [
						unknown,
						unknown,
					][]) {
						map.set(key, value);
