...
export function RouterNode({ routes, errors }: {
    ...
    for (const route of routes) {
        ...
        } else if (route.frameType === "notFound") {
            if (currentNode.notFoundPage) {
                errorList.push(
                    MessageError(
                        `There are multiple different definitions of '${route.path}' with conflicting not-found pages. Each path can only have one not-found page.`,
                    ),
                );
            }

            currentNode.notFoundPage = route.frame;
        }
    }
