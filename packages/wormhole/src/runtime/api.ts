...
import { querySchema, useQuery, type QueryProps, type Signal } from "@vortexjs/core";
...
export function INTERNAL_client_api<Input, Output>(props: ClientAPIProps<Input, Output>): ((inp: Input) => Promise<Output>) & ({
    ...
    if (props.isQuery) {
        const qschema = querySchema({
            id: `wormhole/${props.endpoint}:${props.method}`,
            impl: result,
        });

        (result as any).use = (args: Input, props: QueryProps) => {
            return useQuery(qschema, args, props);
        }
    }
